...
def _import(filename):
    ...
    spec = importlib.util.spec_from_file_location(module_name, filename, submodule_search_locations=[dirname])
...
def assertTrue(logic, *args):
    ...
    info = inspect.getframeinfo(frame)
    msg = []
    msg.append("Assert True failed at %s, line %d, in %s"%(info.filename, info.lineno, info.function))
...
def assertFalse(logic, *args):
    ...
    info = inspect.getframeinfo(frame)
    msg = []
    msg.append("Assert False failed at %s, line %d, in %s"%(info.filename, info.lineno, info.function))
    msg.append(lines[0][frame.f_lineno-lines[1]])
    raise AssertionError("\n".join(msg))
...
def assertEqual(value1, value2, *args):
    ...
    info = inspect.getframeinfo(frame)
    msg = []
    msg.append("Assertion '%s'=='%s' failed at %s, line %d, in %s"%(value1, value2, info.filename, info.lineno, info.function))
    msg.append(lines[0][frame.f_lineno-lines[1]])
    raise AssertionError("\n".join(msg))

...
after an statement has been executed, the method `with_columns
<cursor.with_columns_>`_ can be used to adjust the converters for an individual
query.
...
can also be set on an individual fetch by calling `with_columns <cursor.with_columns_>`_ after
...
calling `with_columns <cursor.with_columns_>`_.
...
.. _cursor.with_columns: #jpype.dbapi2.Cursor.with_columns

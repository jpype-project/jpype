...
public class MethodResolution {
	 ...
	 * @return
	 */
	public static <T extends Executable> List<MethodResolution> sortMethods(List<T> methods) {
	...
	public static boolean isMoreSpecificThan(Executable method1, Executable method2) {
		...
		Class<?> res1 = null;
		Class<?> res2 = null;
		if (method1 instanceof Method) {
			res1 = ((Method) method1).getReturnType();
		} else {
			res1 = Void.TYPE;
		}

		if (method2 instanceof Method) {
			res2 = ((Method) method2).getReturnType();
		} else {
			res2 = Void.TYPE;
		}
		...
		if (method1.isVarArgs() && method2.isVarArgs()) {
			...
			return isMoreSpecificThan(param1, param2, res1, res2);
		}
		...
		if (method1.isVarArgs()) {
			...
			if (n1 - 1 == n2) {
				return isMoreSpecificThan(param1.subList(0, n2), param2, res1, res2);
			}
			...
			if (n1 == n2) {
				...
				boolean isMoreSpecific =
					isMoreSpecificThan(param1, param2, res1, res2) ||
						isMoreSpecificThan(q, param2, res1, res2);
				...
				return isMoreSpecific &&
					!(isAssignableTo(cls2, svCls) && isAssignableTo(svCls, cls2));
			}
			...
			if (n1 < n2) {
				...
				for (int i = n1; i < n2; ++i)
					q.add(cls2);
				return isMoreSpecificThan(q, param2, res1, res2);
			}
		}
		...
		if (method2.isVarArgs()) {
			...
			// Less arguments, chop the list
			if (n2 - 1 == n1) {
				return isMoreSpecificThan(param1, param2.subList(0, n2), res1, res2);
			}
			...
			if (n1 == n2) {
				...
				// Compare both ways
				return isMoreSpecificThan(param1, param2, res1, res2) ||
					isMoreSpecificThan(param1, q, res1, res2);
			}
			...
			if (n2 < n1) {
				...
				return isMoreSpecificThan(param1, q, res1, res2);
			}
		}
	...
	public static boolean isMoreSpecificThan(List<Class<?>> param1, List<Class<?>> param2,
		...
		return isAssignableTo(res1, res2);
	}
}

...
{
	printf("DIRECT (use instance getAttr)\n");
...
JPPyObject JPProxyIndirect::getCallable(PyObject* name, int& addSelf)
{
    printf("INDIRECT (use dispatch map lookup, then instance fallback)\n");
    // Ensure name can be treated as a string key
    if (!PyUnicode_Check(name))
        return JPPyObject::claim(nullptr);

    int has_key = PyMapping_HasKey(m_Instance->m_Dispatch, name);
    if (has_key == 1)
    {
        JPPyObject out = JPPyObject::claim(PyObject_GetItem(m_Instance->m_Dispatch, name));
        if (!out.isNull())
        {
            // Determine if we need to pass 'self' automatically.
            // If it's a bound method coming from _JAttrDictAdapter, Python already bound 'self'.
            // If it's a loose function inside a flat dictionary, we need to supply 'self'.
            addSelf = (m_Instance->m_Target != Py_None) && !PyMethod_Check(out.get());
            return out;
        }
    }
    // Clear any potential KeyError or AttributeError generated by a failed mapping check
    PyErr_Clear(); 

    // 3. Fallback to extracting the attribute directly from the target instance itself
    if (m_Instance->m_Target != Py_None)
    {
        JPPyObject out = JPPyObject::accept(PyObject_GetAttr(m_Instance->m_Target, name));
        addSelf = 0; // Bound attributes automatically resolve 'self'
        if (!out.isNull())
            return out;
        PyErr_Clear();
    }
...
{
	printf("FUNCTIONAL (method, else instance getattr)\n");

...
static bool isNull(PyObject *self)
{
	...
	if (javaSlot == nullptr || javaSlot->getClass()->isPrimitive())
		return false;
	return javaSlot->isJavaNull();
}
...
static Py_hash_t PyJPNumberLong_hash(PyObject *self)
{
	...
	if (!javaSlot->getClass()->isPrimitive() && javaSlot->isJavaNull())
		return Py_TYPE(Py_None)->tp_hash(Py_None);
...
void PyJPNumber_initType(PyObject* module, PyJPModuleState* st)
{
	...
	st->PyJPNumberLong_Type = (PyTypeObject*) PyJPClass_FromSpecWithBases(module, &numberLongSpec, bases.get());
	JP_PY_CHECK(); // GCOVR_EXCL_LINE
	Py_INCREF((PyObject*) st->PyJPNumberLong_Type);
	...
	JP_PY_CHECK(); // GCOVR_EXCL_LINE
	Py_INCREF((PyObject*) st->PyJPNumberFloat_Type);
	...
	st->PyJPNumberBool_Type = (PyTypeObject*) PyJPClass_FromSpecWithBases(module, &numberBooleanSpec, bases.get());
	JP_PY_CHECK(); // GCOVR_EXCL_LINE
	Py_INCREF((PyObject*) st->PyJPNumberBool_Type);
